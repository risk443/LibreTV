...
async function renderDoubanCards(data, container) {
    ...
    if (!data.subjects || data.subjects.length === 0) {
        ...
        for (const item of data.subjects) {
            ...
            // 处理图片URL：先直连豆瓣图片，失败后再切换到带鉴权的本站代理，避免移动端出现全占位
            const originalCoverUrl = item.cover;
            const proxiedCoverUrl = await buildAuthenticatedProxyUrl(originalCoverUrl);
            ...
            card.innerHTML = `
                <div class="relative w-full aspect-[2/3] overflow-hidden cursor-pointer" onclick="fillAndSearchWithDouban('${safeTitle}')">
                    <img src="${originalCoverUrl}" alt="${safeTitle}" 
                        ...
                        data-proxy-src="${proxiedCoverUrl}"
                        onerror="if (!this.dataset.proxyTried) { this.dataset.proxyTried='1'; this.src=this.dataset.proxySrc; } else { this.onerror=null; this.src='image/logo.png'; this.classList.add('object-contain', 'p-6'); }"
...
async function buildAuthenticatedProxyUrl(url) {
    if (!url) return 'image/logo.png';

    const rawProxyUrl = PROXY_URL + encodeURIComponent(url);
    if (window.ProxyAuth?.addAuthToProxyUrl) {
        try {
            return await window.ProxyAuth.addAuthToProxyUrl(rawProxyUrl);
        } catch (error) {
            console.warn('生成图片代理鉴权失败，降级为原图：', error);
        }
    }
